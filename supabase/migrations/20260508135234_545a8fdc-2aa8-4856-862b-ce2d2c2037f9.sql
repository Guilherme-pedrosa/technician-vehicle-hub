ALTER TABLE public.vehicle_checklists ADD COLUMN IF NOT EXISTS freios text NOT NULL DEFAULT 'conforme';
...
BEGIN
  ...
  IF coalesce(NEW.nivel_oleo, '') = ''
    ...
    OR coalesce(NEW.acessorios, '') = ''
    OR coalesce(NEW.freios, '') = '' THEN
