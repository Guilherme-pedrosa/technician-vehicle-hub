...
DECLARE
  required_photo_keys text[] := ARRAY[
    'painel', 'motor', 'nivel_oleo', 'reservatorio_agua',
    'calibracao', 'estepe', 'itens_seguranca',
    'exterior_frente', 'exterior_traseira', 'exterior_esquerda', 'exterior_direita',
    'pneu_de', 'pneu_dd', 'pneu_te', 'pneu_td', 'interior'
  ];
  photo_key text;
BEGIN
  -- Skip validation for drafts (both insert and update)
  IF NEW.status = 'rascunho' THEN
    RETURN NEW;
  END IF;

  -- For UPDATE: only validate if status is changing TO finalizado, or if it's already finalizado
  -- Skip validation for other column updates on already-finalized rows (except status changes)
  IF TG_OP = 'UPDATE' AND OLD.status = 'finalizado' AND NEW.status = 'finalizado' THEN
    RETURN NEW;
  END IF;

  -- Full validation for INSERT (new finalizado) or UPDATE (rascunho → finalizado)
