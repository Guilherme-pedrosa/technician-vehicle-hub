...
-- Add status column to vehicle_checklists
ALTER TABLE public.vehicle_checklists 
ADD COLUMN status text NOT NULL DEFAULT 'finalizado';

-- Allow nullable fields for drafts by making NOT NULL columns nullable where needed
-- We'll handle this in the trigger instead - drafts will use default values

-- Create unique partial index: only 1 draft per vehicle per user
CREATE UNIQUE INDEX idx_one_draft_per_vehicle_user 
ON public.vehicle_checklists (vehicle_id, created_by) 
WHERE status = 'rascunho';

-- Update the validation trigger to skip drafts
...
-- Also run validation on UPDATE when status changes from rascunho to finalizado
-- The current trigger only fires on INSERT, let's also add BEFORE UPDATE
DROP TRIGGER IF EXISTS validate_checklist_complete ON public.vehicle_checklists;
CREATE TRIGGER validate_checklist_complete
  BEFORE INSERT OR UPDATE ON public.vehicle_checklists
  FOR EACH ROW
  EXECUTE FUNCTION public.validate_vehicle_checklist_complete();

-- Remove the duplicate date constraint for drafts (drafts shouldn't count)
-- Check if there's a unique constraint on vehicle_id + checklist_date
